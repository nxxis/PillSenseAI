      ...
      <AlertModal
        ...
        message={
          modal.type === 'reminder'
            ? `Are you sure you want to disable reminders for ${modal.med?.drug}?`
            : modal.type === 'enable-reminder'
            ? `Enable reminders for ${modal.med?.drug} (${
                modal.med?.doseMg
              } mg, ${modal.med?.frequencyPerDay}×/day) at ${
                timeById[modal.med?._id] || '08:00'
              }? You will get notifications at this time.`
            : `Are you sure you want to disable ${modal.med?.drug}? This will end the medication.`
        }
        confirmText="Confirm"
        cancelText="Cancel"
        onConfirm={async () => {
          ...
          if (modal.type === 'reminder') {
            await disableReminder(modal.med);
          } else if (modal.type === 'enable-reminder') {
            await enableReminder(modal.med);
          } else if (modal.type === 'medication') {
            try {
              const res = await fetch(
                `${API_URL}/prescriptions/${modal.med._id}/end`,
